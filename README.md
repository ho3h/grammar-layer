...
A few items the writeup explicitly does not close. The named-feature fingerprint (f15596, f10142) fragments at SAE width 65k while aggregate enrichment stays roughly 70% intact — the underlying mechanism is width-stable but the specific feature names at width 16k are SAE-training artefacts. The behavioural-signature claim was retracted at proper power: GPT-2 means at n=300 are substantially higher than at n=75, the proper-power Pythia-vs-GPT-2 comparison is null on every metric, and the four behavioural metrics we chose do not distinguish inversion-having models from GPT-2 on the open-ended benchmark; a targeted behavioural test conditioning on the copula template (where amplification produces a 10-nat effect) would be the natural follow-up. Auto-interp labels are noisy and the grammar/content keyword classifier is post-hoc. Qwen 3 1.7B and Mistral 7B labels are still unfetched, so the cross-family enrichment claim rests on 4/7 labelled models rather than 7/7. The full limitations section is in the technical writeup.

Items that *were* open at the time of an earlier evaluation and are now closed: the Gemma 2 9B layer-depth confound (closed by re-running at L31/42 = 74% depth — fingerprint recovered), the missing causal amplification experiment (now done across four models, with a Gemma-2-2B-specific negation attractor as the climax), and the missing prior-art engagement with Marks, McDougall, Stolfo, Heimersheim, and Kissane (now a real Related Work section in the writeup).
