...
It is worth saying explicitly which parts of this writeup are *contributions* and which parts are *replications or applications of established machinery*, because the boundary is not where a quick read would put it.

Established prior to this work, applied here: the per-prompt SAE feature attribution via zero-ablation (Marks et al. 2024, "Sparse Feature Circuits"); the concept that specific internal components systematically *oppose* the high-probability completion (Wang et al. 2022 negative name movers; McDougall et al. 2023 copy suppression; Kissane et al. 2024 SAE-level suppression features); the concept that confidence-regulation components exist in language models (Stolfo et al. 2024). The supporting-side joint zero-ablation result (Result 1) and the opposing-side inversion (Result 2) are best read as *adapting* this machinery to a specific task family (capital completions) and a specific feature class (copular template features) — they are sharp claims at the per-prompt level but they do not, on their own, find a mechanism that the prior art could not predict.

The new claims, in descending order of importance:

1. **The negation attractor (Result 5).** Amplifying f15596 in Gemma 2 2B ten times flips the argmax to " not" on all six capital prompts. Amplifying the depth-matched copula feature in Gemma 2 9B, Gemma 1 2B, and Pythia 70M monotonically depresses log P(target) but does *not* produce a directed flip to negation — they converge to " a". This is the specific finding that the McDougall, Stolfo, and Kissane mechanisms explicitly cannot account for: copy suppression suppresses what is already in context, not what is absent; entropy neurons act uniformly via the LN null space and cannot produce a directed flip; token-frequency neurons scale by unigram frequency and " not" is not the right frequency band. The negation attractor is, as far as we can find, a new phenomenon-class observation.

2. **The cross-family copula-suppressor fingerprint (Results 2–3).** The same kind of feature (copula-labelled, on the opposing side of capital prompts) appears at the depth-matched layer in four models across three families and a 130× parameter range, and is absent in GPT-2 small. This extends the SAE-suppression-feature concept of Kissane et al. from a single GPT-2 attention head's analogue to a cross-family pattern.

3. **The capital-template scope.** The fingerprint is specific to capital-completion prompts ("The capital of X is __") and does not extend to other "X is Y" templates (currencies, languages, continents, compositions). This is a tighter scope claim than would be predicted from the copy-suppression literature, which says nothing about prompt-template specificity.

4. **A worked example of Heimersheim's population-statistics illusion caught in the work's own pipeline.** The v2 → v3 correction reclassified the same feature population from "supporting backbone" to "opposing suppressor" once the per-prompt slice was held fixed during aggregation. The behavioural retraction at n=300 is a second instance of the same intellectual move applied to a downstream claim. We treat the methodology of catching these as part of the contribution.

What is explicitly *not* a contribution: the existence of suppression features in language models, the existence of confidence-regulation mechanisms, the SAE-attribution methodology itself, or the broad finding that grammar features participate in next-token prediction.
...
In Gemma 2 2B specifically — and only Gemma 2 2B, not Gemma 2 9B at the depth-matched layer, not Gemma 1 2B, not Pythia 70M — scaling the copula feature ten times flips the argmax through generic completions to outright negation on every single capital prompt. We verified the mechanism on the decoder side: " not" (token id 780) ranks **2nd of all 256,000 unembedding rows** by cosine similarity with f15596's decoder direction (cos = +0.178), behind only " able" (cos = +0.178). The next eight rows are " actually", " worth", " designed", " seen", " going", " still", " considered", " always" — a cluster of predicate-state modal tokens centred on negation and modality. The decoder vector is not random with respect to the unembedding: it has substantial structure aligned with the very class of tokens the argmax ends up at when we amplify. A random feature's decoder direction (control: f5000) has cos = −0.027 with " not", and its top-K is unrelated to this cluster. The amplification result is therefore mechanistically explained: f15596 is encoding negation polarity in its decoder direction, in addition to the copular template signal that drives its activation pattern on copula tokens.
